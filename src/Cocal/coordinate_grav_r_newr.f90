...
!
! ==1145329==WARNING: MemorySanitizer: use-of-uninitialized-value
    ratrf = 0.1d0
!
...
! 
    ...
    if (rgmid > 2.0d0) then
      nrf = int(0.64*nrf)
      rgcon = 9.0d0  !  => Max sepa=8 or dis_max=4   =>  rgcon=sepa+1
    else
      rgcon = 5.0d0   !  => Max sepa=4 or dis_max=2   =>  rgcon=sepa+1
    end if
!
    drdr =  1.0d0/dble(nrf)
    drdrinv = 1.0d0/drdr
!
    nr1 = nrf + int((1.25d0-1.0d0)/drdr)    ! Up until r=1.25  dr=1/nrf    OR 1.25*nrf intervals
...
!   Find rgmid
    DO ir = 1, nr2
      if ((rg(ir)-1.0d-10)<=rgmid .and. rgmid<(rg(ir+1)-1.0d-10)) then
        if (dabs(rg(ir)-rgmid)<=1.0d-13) then
          nrgin = ir
          exit
        else
